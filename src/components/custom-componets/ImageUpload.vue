<template>
  <div class="image-upload">
    <div class="imgHeader"
         ...
         :key="index">
      <i v-if="pageAction === 'info'"
         class="ifont"></i>
      <i v-if="pageAction === 'edit'"
         @click="detailImg(index)"
         class="el-icon-close ifont"></i>
      <img :src="img"
           alt="未获取到图片"
           :width="width"
           :height="height"
    ...
    <label v-if="previewImages.length < limit && mode !== 'viewImg'"
           :for="name+ 'uploader'"
...
<script>
export default {
  props: ['name', 'uploaderMode', 'externalPreviewImages', 'limit', 'accept', 'uploadUrl', 'success', 'width', 'height', 'pageAction2'],
  data() {
    return {
      files: [],
      ...
      new Promise((resolve, reject) => {
        ...
        if (!isLt200k) {
          ...
          this.photoCompress(
            ...
            function (base64Codes) {
              ...
              if (that.replaceIndex !== -1) {
                ...
                that.previewImages.splice(that.replaceIndex, 1, window.URL.createObjectURL(file))
          ...
          if (this.replaceIndex !== -1) {
            ...
            this.previewImages.splice(this.replaceIndex, 1, window.URL.createObjectURL(file))
            ...
            if (xhr.readyState === 4) {
              if (xhr.readyState === 4 && xhr.status === 200) {
                ...
                if (imageRes.ret) {
                  ...
                  this.uploadRes[i] = ''
                  this.$message.error('第' + (i + 1) + '张图片上传失败：' + imageRes.error.message)
                }
                this.uploadedCount++
                if (this.uploadedCount === this.uploadRes.length) {
                  let res = new Array(this.replacedImg.length)
                  if (this.mode === 'replace') {
                    for (var k = 0, j = 0; k < this.replacedImg.length && j < this.uploadRes.length; k++) {
