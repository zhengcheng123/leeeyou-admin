<template>
  <div class="store-all">
        ...
        <div class="section-body basic-info">
          ...
          >
            <el-form-item label="店铺名称：" prop="name">
              ...
              <el-input v-else v-model.trim="itemForm.name"></el-input>
            </el-form-item>
            <el-form-item label="店铺地址：" prop="address">
              <span v-if="pageAction==='info'">{{itemForm.address}}</span>
              <el-input v-else v-model.trim="itemForm.address"></el-input>
            </el-form-item>
            <el-form-item label="店铺图标：" prop="icon">
              <image-upload
                :pageAction2="pageAction"
                name="icon"
            ...
            <el-form-item label="客服电话：" prop="phones">
              <span v-if="pageAction==='info'">{{itemForm.phones}}</span>
              <el-input v-else v-model.trim="itemForm.phones"></el-input>
            </el-form-item>
          </el-form>
        </div>
      </div>
          ...
          <el-table :data="tableData" style="width: 100%" :row-class-name="tableRowClassName">
            ...
            >
              <template slot-scope="scope">
                <div class="tableAdd" v-if="scope.row.sort == '+'">
                  <p @click="changeName('add')">{{scope.row.sort}}</p>
                </div>
                <div class="tablebtn" v-else>{{scope.row.index}}</div>
              </template>
            </el-table-column>
            <el-table-column prop="name" label="名称" width="280" :resizable="false">
              <template slot-scope="scope">
                <div class="tablebtn">
                  <el-button
                    type="text"
                    class="btn"
                    @click="changeName('edit',scope.row)"
                  >{{scope.row.name}}</el-button>
                </div>
              </template>
            </el-table-column>
            <el-table-column prop="name" label="操作" width="150" :resizable="false">
              <template slot-scope="scope">
                <div class="table_select">
                  <div class="tablebtn" v-if="scope.row.sort == '+'"></div>
                  <div class="tablebtn" v-else @click="changedelte(scope.row)">
      ...
      <!-- <div class="form-section">
        ...
        <div class="section-body pay-section">
          ...
          <div class="item" v-if="prepareAddPay">
            <el-row>
              <el-col :span="6">
                <el-select v-model="prepareAddPay.payId" size="small">
                  <el-option
              ...
              <el-col :span="9" :offset="1">
                <el-input size="small" v-model.trim="prepareAddPay.payAccount"></el-input>
              </el-col>
              <el-col :span="6" :offset="2">
                <el-button size="small" @click="addPay" :loading="saving">添加</el-button>
              </el-col>
            </el-row>
          </div>
        </div>
    ...
    >
      <el-form :model="ruleForm" :rules="rules" ref="ruleFormName" class="ruleFormc">
        <el-form-item prop="name">
          <el-input v-model="ruleForm.name" autocomplete="off" placeholder="请输入名称" maxlength="10"></el-input>
          <p>不超过10个字符</p>
        </el-form-item>
      </el-form>
      <span slot="footer" class="dialog-footer">
        <el-button @click="changeguanbi">取 消</el-button>
        <el-button type="primary" @click="changenameDetermine('ruleFormName')">确 定</el-button>
      </span>
    </el-dialog>
    <!-- 删除弹框 -->
    <el-dialog title="删除类别" :visible.sync="deteledialog" width="20%" :close-on-click-modal="false">
      <span>删除类别时请确认，该类别下无对应商品是否删除该类别？</span>
      <span slot="footer" class="dialog-footer">
        <el-button @click="deteledialog = false">取 消</el-button>
        <el-button type="primary" @click="deteleTab()">确 定</el-button>
      </span>
    </el-dialog>
  </div>
</template>
    ...
    var updateFunc = function(event) {
      ...
      that.$nextTick(function() {
        ...
        that.$nextTick(function() {
          ...
          new Sortable(
            that.$el.querySelector(".el-table__body-wrapper tbody"),
            {
