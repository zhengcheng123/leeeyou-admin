  ...
  <div>
    <div class="setStore_city">
        ...
        <div class="form-section">
          <div class="section-head">
            ...
            <div class="button-address">
              ...
              <el-button type="text" v-else @click="addLogisticsCompanyItem">
          ...
          <div class="section-body">
            <div class="section-set">
              <div v-for="company in logisticsCompanies" class="logistics-row">
                <!-- <span v-if="company.type===1">发货地址</span> -->
                <div v-if="company.type===1">
                  物流公司：
                  <el-input v-if="company.hasEdit" v-model="company.name"></el-input>
                  <p class="edit-p" v-else>{{company.name}}</p>
                </div>
                <div v-if="company.type===1">
                  发货地址：
                  <el-input v-if="company.hasEdit" v-model="company.address"></el-input>
                  <p class="edit-p" v-else>{{company.address}}</p>
                </div>
                ...
                <div :offset="1">
                  联系人：
                  <el-input v-if="company.hasEdit" v-model="company.contactsName"></el-input>
                  <p class="edit-p" v-else>{{company.contactsName}}</p>
                </div>
                <div :offset="1">
                  联系方式：
                  <el-input v-if="company.hasEdit" v-model="company.contactsMobile"></el-input>
                  <p class="edit-p" v-else>{{company.contactsMobile}}</p>
                </div>
                <div class="button-eid">
                  <el-button @click="editSet(company)" class="button-ico" type="primary">编辑</el-button>
                  <el-button @click="savesetStore(company)" class="button-ico" type="primary">保存</el-button>
                  <el-button
                    v-if="company.type===2"
                    icon="el-icon-delete"
                    size="mini"
                    ...
                    class="button-ico"
                    plain
                    @click="deleteLogisticsCompany(company.id)"
                  ></el-button>
                </div>
              </div>
            </div>
            ...
            >
              ...
              <el-form-item v-if="logisticsCompanyForm.type===1" label="物流公司" prop="name">
                <el-input size="small" v-model.trim="logisticsCompanyForm.name"></el-input>
              </el-form-item>
              <el-form-item v-if="logisticsCompanyForm.type===1" label="出货地址" prop="name">
                <el-input size="small" v-model.trim="logisticsCompanyForm.address"></el-input>
              </el-form-item>
              <el-form-item label="自提地址" prop="address">
                <el-input size="small" v-model.trim="logisticsCompanyForm.address"></el-input>
              </el-form-item>
              <el-form-item label="联系人" prop="contactsName">
                <el-input size="small" v-model.trim="logisticsCompanyForm.contactsName"></el-input>
              </el-form-item>
              <el-form-item label="联系方式" prop="contactsMobile">
                <el-input size="small" v-model.trim="logisticsCompanyForm.contactsMobile"></el-input>
              </el-form-item>
              <el-form-item>
                <el-button size="small" type="primary" @click="addLogisticsCompany">添加</el-button>
              </el-form-item>
            </el-form>
          </div>
        </div>
        <div class="form-section">
          ...
          <section class="section" style="background:#fff">
            ...
            >
              ...
              >
                <template slot-scope="scope">
                  <div style="cursor:pointer" class="popover-father">
                    <span v-if="scope.$index===0" v-html="'默认运费'"></span>
                    <div v-else>
                      <el-popover
                        :disabled="!scope.row.addresses.length"
                        placement="top-start"
                        width="400"
                        trigger="hover"
                      ...
                      >
                       <span
                        ...
                        @click="editPlace(scope.row,scope.$index)"
                      >编辑</span>
                        <span
                          style="display:block"
                          slot="reference"
                          v-if="!scope.row.hasAddressEdit "
                        >{{scope.row.addresses.map(e=>{return e.address}).join('，')}}</span>
                      </el-popover>
              ...
              <el-table-column prop="firstWeight" label="首重(kg)" min-width="150" align="center">
                <template slot-scope="scope">
                  <el-input
                    v-model="scope.row.firstWeight"
                    @keyup.native="checkNumber(scope.row,'firstWeight')"
                    :disabled="isEditObj[scope.$index][`edit${scope.$index}`]"
                  ></el-input>
                </template>
              </el-table-column>
              <el-table-column prop="firstPrice" label="首重运费（元）" min-width="150" align="center">
                <template slot-scope="scope">
                  <el-input
              ...
              <el-table-column prop="secondWeight" label="续重（kg）" min-width="150" align="center">
                <template slot-scope="scope">
                  <el-input
                    @keyup.native="checkNumber(scope.row,'secondWeight')"
                    v-model="scope.row.secondWeight"
                    :disabled="isEditObj[scope.$index][`edit${scope.$index}`]"
                  ></el-input>
                </template>
              </el-table-column>
              <el-table-column prop="secondPrice" label="续重运费（元）" min-width="150" align="center">
                <template slot-scope="scope">
                  <el-input
                    @keyup.native="checkNumber(scope.row,'secondPrice')"
                    v-model="scope.row.secondPrice"
                    :disabled="isEditObj[scope.$index][`edit${scope.$index}`]"
                  ></el-input>
                </template>
              </el-table-column>
              <el-table-column prop="freePrice" label="包邮条件(满多少元包邮)" min-width="150" align="center">
                <template slot-scope="scope">
                  <el-input
                    @keyup.native="checkNumber(scope.row,'freePrice')"
                ...
                <template scope="props">
                  <div class="btn-group">
                    ...
                    <el-button
                      class="age-btn"
                      type="text"
                      @click="saveTable(props.row,props.$index)"
                    >保存</el-button>
                    <el-button
                      class="age-btn"
                      type="text"
                      v-if="!props.row.def"
                      @click="detailForm(props.row,props.$index)"
                    >删除</el-button>
                  </div>
                </template>
              </el-table-column>
            ...
            <div class="row">
              <el-form :inline="true">
                <el-form-item>
                  <el-button type="primary" @click="addStore()" class="f-left">添加规则</el-button>
                </el-form-item>
              </el-form>
            </div>
      ...
      <el-dialog
        ...
        @close="resetForm"
      >
        <div v-for="(city,index) in cityDatas" :key="index">
          <div class="city-children">
            <el-checkbox
              :disabled="city.disabled"
              :indeterminate="city.indeterminate"
              v-model="city.provinceSelectAll"
              @change="(val)=>handleCheckAllChange(val,city)"
            >{{city.province}}</el-checkbox>
          </div>
          <div class="city-children2">
            ...
            >
              <el-checkbox
                v-for="(cityChildren,ind) in city.cities"
                :disabled="cityChildren.disabled"
                :label="cityChildren"
                :key="ind"
              >{{cityChildren.city}}</el-checkbox>
            </el-checkbox-group>
          </div>
        </div>
        <span slot="footer" class="dialog-footer">
          <el-button @click="cancelCity()">取 消</el-button>
          <el-button type="primary" @click="saveCity()">确 定</el-button>
        </span>
      </el-dialog>
    </div>
  </div>
...
export default {
  data() {
    var validateCheckPhone = (rule, value, callback) => {
      if (!value) {
        ...
        if (
          !/^1[3456789]\d{9}$/.test(this.logisticsCompanyForm.contactsMobile)
        ) {
  ...
  methods: {
    ...
    getData() {
      this.$axios
        .post(globalConfig.server1 + "postpage/query", this.params)
        .then(res => {
    ...
    editPlace(row, index) {
      ...
      this.$axios
        .get(`${globalConfig.server1}postpage/getUsingAddress`)
        .then(res => {
          if (res.data.result === 1) {
            this.loadData();
    ...
    handleCheckAllChange(val, province) {
      ...
      if (val) {
        ...
        this.checkedCities = this.getUniqueArray(
          this.checkedCities,
          "city_code"
        );
      } else {
    ...
    handleCheckedCitiesChange(value, province) {
      ...
      province.indeterminate =
        checkedCount > 0 && checkedCount < optionsArr.length;
    ...
    savesetStore(company) {
      ...
      if (
        ...
        this.$axios
          .post(
            globalConfig.server1 + "api/store/updateLogisticsCompany",
            company
          )
          .then(res => {
            if (res.data.result === 1) {
      ...
      if (row.id == null) {
        ...
        this.$confirm("确定删除该条角色信息?", "提示", {
          ...
          .then(() => {
            let ids = [row.id];
            this.$axios
              .post(`${globalConfig.server1}postpage/delete`, [row.id])
              .then(res => {
                if (res.data.result === 1) {
                  this.$message.success(res.data.msg);
                  this.cityDatas = [];
