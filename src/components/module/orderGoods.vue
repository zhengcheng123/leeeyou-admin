  ...
  <div>
    ...
    <div class="content">
      ...
      <el-table :data="currentItems" @selection-change="selectionChange" border style="width: 100%"
        ...
        <el-table-column prop="trade_id" label="交易号" sortable>
        </el-table-column>
        <el-table-column prop="goods_id" label="商品号" sortable>
        </el-table-column>
        <el-table-column prop="num" label="购买数量" sortable>
        </el-table-column>
        <el-table-column prop="real_price" label="应付款" sortable>
        </el-table-column>
        <el-table-column prop="pay_price" label="实际付款" sortable>
        </el-table-column>
        <el-table-column prop="stat" label="单个物品的交易状态,1：未付款，2" sortable>
        </el-table-column>
      ...
      <el-dialog title="编辑" :visible.sync="itemFormVisible" size="tiny" custom-class="account-dialog"
        ...
        <el-form :model="itemForm" ref="itemForm" label-width="70px" class="account-form"
          ...
          <el-form-item label="交易号" prop="tradeId">
            <el-input v-model.number="itemForm.tradeId"></el-input>
          </el-form-item>
          ...
          <el-form-item label="应付款" prop="realPrice">
            <el-input v-model.number="itemForm.realPrice"></el-input>
          </el-form-item>
          <el-form-item label="实际付款" prop="payPrice">
            <el-input v-model.number="itemForm.payPrice"></el-input>
          </el-form-item>
          <el-form-item label="单个物品的交易状态,1：未付款，2" prop="stat">
            <el-input v-model.number="itemForm.stat"></el-input>
          </el-form-item>
  ...
  export default {
    data () {
      return {
        ...
        itemFormRules: {
          ...
          realPrice: [
            {required: true, message: '请输入应付款'},
            {type: 'number', message: '值必须为数字'}
          ],
          payPrice: [
            {required: true, message: '请输入实际付款'},
            {type: 'number', message: '值必须为数字'}
          ],
          stat: [
            {required: true, message: '请输入单个物品的交易状态,1：未付款，2'},
            {type: 'number', message: '值必须为数字'}
          ]
        },
        /* table */
    ...
    methods: {
      ...
      editItem (item) {
        ...
        this.itemForm.tradeId = item.trade_id
        this.itemForm.goodsId = item.goods_id
        this.itemForm.num = item.num
        this.itemForm.realPrice = item.real_price
        this.itemForm.payPrice = item.pay_price
        this.itemForm.stat = item.stat
        this.itemFormVisible = true
      },
      saveItem () {
        this.$refs['itemForm'].validate(valid => {
          if (valid) {
            this.saving = true
            let url = 'api/orderGoods/add'
            if (this.itemForm.id !== -1) {
              url = 'api/orderGoods/update'
            }
            this.$http.ajax({
              url: APIHOST + url,
      ...
      deleteItem (id) {
        this.$http.ajax({
          url: APIHOST + 'api/orderGoods/delete',
          type: 'get',
          data: {id: id},
          dataType: 'json',
          context: this
      ...
      getItems () {
        this.$http.ajax({
          url: APIHOST + 'api/orderGoods/list',
