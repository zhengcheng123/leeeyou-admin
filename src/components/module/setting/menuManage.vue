<template>
  <div class="wrapper menu-manage" :key="key">
    <el-container>
      <el-aside width="300px" hight="100%" :style="boxHeight">
        <div class="btn-group">
          <!-- v-if="$_has('menu-resourceInsert-menu-resource')" -->
          <el-button class="f-left" type="primary" size="small" @click="addItems(0)" :disabled="disabledfalse">新增</el-button>
          ...
          class="f-left"
            ...
            size="small"
            @click="editItems(0)"
            :disabled="disabledfalse || clicknode.id === '1'"
          >编辑</el-button>
          ...
          <el-button class="f-left" type="primary" size="small" @click="sortClick">排序</el-button>
        </div>
        <el-scrollbar>
          <div class="menu-tree">
            <el-tree
      ...
      <section>
            ...
            <el-form-item style="margin-right: 0;">
              ...
              <el-button
               class="f-left"
                ...
                size="small"
                @click="deleteItems(1)"
                :disabled="idBatch.length>0?false:true"
              >删除</el-button>
            </el-form-item>
          </div>
        </el-form>
        ...
        >
          ...
          <el-table-column align="center" prop="resourceKey" show-overflow-tooltip label="资源值"></el-table-column>
          <el-table-column align="center" prop="path" show-overflow-tooltip label="资源地址"></el-table-column>
          <el-table-column align="center" prop="name" show-overflow-tooltip label="资源名称"></el-table-column>
        </el-table>
        <footer>
          <el-pagination
            background
    ...
    >
      <el-form :model="form" ref="form" label-width="110px">
        <el-form-item label="菜单层级">
          <el-col :span="22">
            <el-input v-model="form.grade" size="small" disabled></el-input>
          </el-col>
        </el-form-item>
        <el-form-item label="上级菜单名称" v-show="form.parentId !== '1' && menuTitle === '新增菜单'">
          <el-col :span="22">
            <el-input v-model="form.parentName" size="small" disabled></el-input>
          </el-col>
        </el-form-item>
        <el-form-item label="菜单名称" required>
          <el-col :span="22">
            <el-input v-model.trim="form.name" size="small" clearable></el-input>
          </el-col>
        </el-form-item>
        <!--  -->
        <el-form-item label="菜单图标" required v-show="form.parentId === '1' || form.grade === '一级菜单'">
          <el-col :span="22">
            <el-input v-model.trim="form.icon" size="small"></el-input>
          </el-col>
        </el-form-item>
        <el-form-item
          label="菜单路径"
        ...
        >
          <el-col :span="22">
            <el-input v-model.trim="form.path" size="small"></el-input>
          </el-col>
        </el-form-item>
        <el-form-item label="首页封面" required v-if="form.grade=='一级菜单'">
          <el-col :span="22">
            <el-upload
      ...
      <div slot="footer" class="dialog-footer">
        <el-button size="small" type="success" @click="sureSubmit" :loading="submitLoading">提 交</el-button>
        <el-button @click="addMenuDialogVisible = false" type="info">取 消</el-button>
      </div>
    </el-dialog>
    <!-- 资源弹窗 -->
    <el-dialog
    ...
    >
      <el-form :model="form" label-width="100px">
        ...
        <el-form-item label="资源值" required>
          <el-col :span="22">
            <el-input v-model="form.resourceKey" size="small"></el-input>
          </el-col>
        </el-form-item>
      ...
      <div slot="footer" class="dialog-footer">
        <el-button type="success" @click="sureSubmit" :loading="submitLoading">提 交</el-button>
        <el-button @click="addUserDialogVisible = false" type="info">取 消</el-button>
      </div>
    </el-dialog>
    <!-- 排序弹窗 -->
    <el-dialog
    ...
    >
      ...
      ></el-tree>
      <div slot="footer" class="dialog-footer">
        <el-button type="success" @click="sortgaibianTrue" :loading="submitLoading">提 交</el-button>

        <el-button @click="sortdialogFormVisible = false" type="info">取 消</el-button>
      </div>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getTableItems() {
      this.$axios
        .post(globalConfig.server1 + "menu-resource/query", this.menuForm)
        .then(res => {
          if (res.data.code === 200) {
    ...
    editItems(type) {
      ...
      if (type) {
        ...
        this.form.pic = this.clicknode.pic
          ? globalConfig.server1 + this.clicknode.pic.substring(1)
          : "";
        this.addMenuDialogVisible = true;
      }
    },
      ...
      if (list.length) {
        this.$confirm(
          ...
          .then(() => {
            this.$axios
              .post(
                globalConfig.server1 + "menu-resource/delete",
                 list 
              )
              .then(res => {
                if (res.data.code === 200) {
    ...
    sureSubmit() {
      ...
      if (this.menuTitle === "新增菜单" || this.resourceTitle === "新增资源") {
        this.$axios
          .post(
            globalConfig.server1 + "menu-resource/saveOrUpdate",
            this.form
          )
          .then(res => {
            if (res.data.code === 200) {
    ...
    handleDrop(draggingNode, dropNode, dropType, ev) {
      ...
      let arr = dropNode.parent.data.children
        ...
        : dropNode.parent.data; //父级下的新数据排序
      let arr1 = [];
      // return false;
    ...
    sortgaibianTrue() {
      if (this.sortCan.length == 0) {
        ...
        this.submitLoading = true;
        this.$http
          .post(globalConfig.server1 + "menu-resource/sort-menu", this.sortCan)
          .then(res => {
            if (res.data.code == "200") {
              this.$message({
                message: "修改成功，请重新登录",
