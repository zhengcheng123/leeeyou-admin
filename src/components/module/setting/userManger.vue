    ...
    <div class="content">
      ...
      >
        ...
        >
          ...
          <el-table-column min-width="120" prop="enable" align="center" label="启用状态">
            <template slot-scope="props">
              <el-switch
                ...
                @change="statusSwitch($event, props.row)"
              ></el-switch>
            </template>
          </el-table-column>
        </el-table>
      ...
      >
        ...
        >
          <el-form-item label="账号" prop="name">
            ...
            <el-col :span="22">
              <el-input
                v-model.trim="userInfoForm.realName"
                clearable
                size="small"
                placeholder="请输入真实姓名"
                autocomplete="off"
          ...
          <el-form-item label="证件类型" prop="authType">
            <el-col :span="22">
              <el-select clearable v-model="userInfoForm.authType">
                <el-option
             ...
             <el-form-item label="证件号" prop="authAccount">
            <el-col :span="22">
              <el-input
                v-model.trim="userInfoForm.authAccount"
                clearable
                size="small"
                placeholder="请输入证件号"
                autocomplete="off"
          ...
          <el-form-item label="确认密码" prop="checkPass">
            <el-col :span="22">
              <el-input
                v-model.trim="userInfoForm.checkPass"
                show-password
                size="small"
                placeholder="请再次输入密码"
                autocomplete="new-password"
          ...
          <el-form-item label="角色" prop="roles">
            <el-col>
              <el-select class="palte" v-model="userInfoForm.roles"  multiple placeholder="请选择">
                <el-option
...
export default {
  ...
  data() {
    ...
    var validateCheckId = (rule, value, callback) => {
      if (!value) {
        ...
        if (
          !/(^\d{15}$)|(^\d{17}([0-9]|X))$/.test(this.userInfoForm.authAccount)
        ) {
    ...
    return {
      ...
      rules: {
        ...
        authAccount: [
          { required: true, validator: validateCheckId, trigger: "blur" }
        ],
        checkPass: [
  ...
  searchData() {
    ...
    getTableItems() {
      ...
      this.$axios
        .post(`${globalConfig.server1}user/query`, this.conditionForm)
        .then(res => {
    ...
    if (!filter.test(this.userInfoForm.qq)&&this.userInfoForm.qq) {
      ...
      this.$refs["userInfoForm"].validate(valid => {
        if (valid) {
          ...
          this.$axios
            .post(`${globalConfig.server1}${url}`, params)
            .then(res => {
              if(res.data.code === 200){
    ...
    deleteItems() {
      this.$confirm("确定删除该条账号信息?", "提示", {
        ...
        .then(() => {
          ...
          this.$axios
            .post(`${globalConfig.server1}user/delete`, arr)
            .then(res => {
              this.$message[res.data.code === 200 ? "success" : "error"](
                res.data.message
              );
    ...
    getRole() {
      this.$axios
        .post(`${globalConfig.server1}role/query`, this.params)
        .then(res => {
          if (res.data.code === 200) {
