          ...
          <el-table-column width="240"
            ...
            <template slot-scope="props">
              <el-button type="text"
                         class="success"
                         size="mini"
                         @click="getRootTreeInfo(props.row)">设置权限</el-button>
            </template>
          </el-table-column>
        </el-table>
      </section>
      ...
      <el-dialog width="400px"
        ...
        <el-form :model="roleInfoForm"
          ...
          <el-form-item label="角色名称"
                        prop="roleName">
            <el-col :span="22">
              <el-input v-model.trim="roleInfoForm.roleName"
                        clearable
                        size="mini"
                        placeholder="请输入名称"
          ...
          <el-form-item label="角色描述"
                        prop="roleDesc">
            <el-col :span="22">
              <el-input v-model="roleInfoForm.roleDesc"
                        clearable
                        size="mini"
                        placeholder="请输入描述"
                        autocomplete="off"></el-input>
      ...
      <el-dialog width="400px"
        ...
        <div slot="footer"
          ...
          <el-button size="mini"
                     type="primary"
                     :loading="submitLoading"
                     @click="submitRootSetting">提 交</el-button>
          <el-button type="info"
                     size="mini"
                     @click="dialogRootVisible = false">取 消</el-button>
        </div>
      </el-dialog>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    getTableItems() {
      ...
      this.$axios.post(`${globalConfig.server1}role/query`, this.conditionForm).then((res) => {
    ...
    submitFom() {
      this.$refs['roleInfoForm'].validate((valid) => {
        if (valid) {
          ...
          this.$axios.post(`${globalConfig.server1}${url}`, this.roleInfoForm).then((res) => {
            this.getTableItems()
    ...
    deleteItems() {
      this.$confirm('确定删除该条角色信息?', '提示', {
        ...
        .then(() => {
          ...
          this.$axios.post(`${globalConfig.server1}role/delete`, arr).then((res) => {
            this.$message[res.data.code === 200 ? 'success' : 'error'](res.data.message)
            this.getTableItems()
          })
        })
    ...
    getRole() {
      ...
      this.$axios.post(`${globalConfig.server1}menu-resource/select-menu-resource-roles`, [this.getRoleId]).then((res) => {
        if (res.data.code === 200) {
          let rootData = []
     ...
     */
    submitRootSetting() {
      let datas = this.$refs.tree.getCheckedNodes(false, true).map((ele) => ele.id)
      this.$axios
