  ...
  <div>
    <div class="content">
      ...
      <el-table :data="currentItems" @selection-change="selectionChange" border style="width: 100%"
        ...
        <el-table-column prop="value" label="商品属性值" sortable>
        </el-table-column>
        <el-table-column prop="pic" label="如果有图片，value为悬浮图片展示的值；则填写这个字段否则为空" sortable>
        </el-table-column>
      ...
      <el-dialog title="编辑" :visible.sync="itemFormVisible" size="tiny" custom-class="account-dialog"
        ...
        <el-form :model="itemForm" ref="itemForm" label-width="70px" class="account-form"
          ...
          <el-form-item label="商品属性值" prop="value">
            <el-input v-model.trim="itemForm.value"></el-input>
          </el-form-item>
          <el-form-item label="如果有图片，value为悬浮图片展示的值；则填写这个字段否则为空" prop="pic">
    ...
    data () {
      return {
        ...
        itemFormRules: {
          ...
          value: [
            {required: true, message: '请输入商品属性值'}
          ],
          pic: [
            {required: true, message: '请输入如果有图片，value为悬浮图片展示的值；则填写这个字段否则为空'}
          ]
        },
        /* table */
        currentItems: [],
    ...
    methods: {
      ...
      saveItem () {
        this.$refs['itemForm'].validate(valid => {
          if (valid) {
            this.saving = true
            let url = 'api/goods2attrs/add'
            if (this.itemForm.id !== -1) {
              url = 'api/goods2attrs/update'
            }
            this.$http.ajax({
              url: APIHOST + url,
      ...
      deleteItem (id) {
        this.$http.ajax({
          url: APIHOST + 'api/goods2attrs/delete',
          type: 'get',
          data: {id: id},
          dataType: 'json',
          context: this
      ...
      getItems () {
        this.$http.ajax({
          url: APIHOST + 'api/goods2attrs/list',
