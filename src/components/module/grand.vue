  ...
  <div>
    ...
    <div class="content">
      ...
      <el-table :data="currentItems" @selection-change="selectionChange" border style="width: 100%"
        ...
        <el-table-column prop="icon" label="品牌图标" sortable>
        </el-table-column>
        <el-table-column prop="company" label="公司名称" sortable>
        </el-table-column>
        <el-table-column prop="introduce" label="品牌简介" sortable>
        </el-table-column>
        <el-table-column prop="py" label="品牌首字母拼音大写" sortable>
        </el-table-column>
      ...
      <el-dialog title="编辑" :visible.sync="itemFormVisible" size="tiny" custom-class="account-dialog"
        ...
        <el-form :model="itemForm" ref="itemForm" label-width="70px" class="account-form"
          ...
          <el-form-item label="品牌图标" prop="icon">
            <el-input v-model.trim="itemForm.icon"></el-input>
          </el-form-item>
          <el-form-item label="公司名称" prop="company">
            <el-input v-model.trim="itemForm.company"></el-input>
          </el-form-item>
          <el-form-item label="品牌简介" prop="introduce">
            <el-input v-model.trim="itemForm.introduce"></el-input>
          </el-form-item>
          <el-form-item label="品牌首字母拼音大写" prop="py">
            <el-input v-model.trim="itemForm.py"></el-input>
          </el-form-item>
  ...
  export default {
    data () {
      return {
        ...
        itemFormRules: {
          ...
          py: [
            {required: true, message: '请输入品牌首字母拼音大写'}
          ]
        },
        /* table */
        currentItems: [],
    ...
    methods: {
      ...
      editItem (item) {
        ...
        this.itemForm.icon = item.icon
        this.itemForm.company = item.company
        this.itemForm.introduce = item.introduce
        this.itemForm.py = item.py
        this.itemFormVisible = true
      },
      saveItem () {
        this.$refs['itemForm'].validate(valid => {
          if (valid) {
            this.saving = true
            let url = 'api/grand/add'
            if (this.itemForm.id !== -1) {
              url = 'api/grand/update'
            }
            this.$http.ajax({
              url: APIHOST + url,
