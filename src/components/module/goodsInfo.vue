  ...
  <div>
    ...
    <div class="content">
      <div class="row goods-row">
        <el-button class="f-left" @click="$router.push('/api/goods')">返回</el-button>
        <el-button v-if="pageAction ==='info'" class="f-left" @click="$router.push('edit')">
          <i class="el-icon-edit"></i>编辑
        </el-button>
        <el-button v-if="pageAction === 'edit'" class="f-left" @click="updateGoods">
      ...
      <div class="form-section">
        ...
        <div class="section-body">
          <el-form :model="infoGood" ref="goodsCommonForm" label-width="100px">
            <el-form-item label="详细类别：" prop="typeId">
              <span v-if="pageAction === 'info'">{{type}}</span>
              <el-radio-group v-else v-model="categoryTypeId" @change="categorychange(categoryTypeId)">
              <el-radio class="is-bordered" v-for="item in categoryType" :label="item.id" :key="item.id" :value="item.id">
                 {{item.typeName}}
              </el-radio>
              </el-radio-group>
            </el-form-item>
            <el-form-item label="商品品牌：" prop="grandId">
              <span v-if="pageAction === 'info'">{{grandId.name}}</span>
               <el-select v-else v-model.trim="grandId.id" size="small" placeholder="请选择品牌">
                      <el-option
                        v-for="(item,index) in grands"
                        :key="index"
                        :label="item.name"
                        :value="item.id"
                      ></el-option>
            ...
            <el-form-item label="商品名称：" prop="name">
              ...
              <el-input disabled  v-else size="small" class="select-input" v-model.trim="infoGood.name"></el-input>
            </el-form-item>
            <el-form-item label="商品描述：" prop="description">
              ...
              <el-input
                ...
                class="select-input"
                size="small"
                v-model.trim="infoGood.description"
              ></el-input>
            </el-form-item>
          </el-form>
          <div class="item">
            ...
            <div class="item-body">
              <image-upload
                :externalPreviewImages="goodImgs"
      ...
      <div class="form-section">
        ...
        <div class="section-body" v-if="pageAction ==='edit'">
          <div class="item">
            ...
            <div class="item-body">
              <div class="formats">
                <div class="select-attr">
                  ...
                  >
                    <el-option
                      v-for="goodAttr in goodAttrs"
                      :key="goodAttr.id"
                      :label="goodAttr.name"
                      :value="goodAttr"
                    ></el-option>
                  </el-select>
                  <!-- <el-button @click="addPredictAttr" size="small" :disabled="isEmpty">添加</el-button> -->
                  <!-- <el-button v-if="!isEmpty" @click="addPredictAttr" size="small">添加</el-button> -->
                </div>
                <ul>
                  <li class="predict-attr" v-for="(predictAttr,index) in predictAttrs" :key="index">
                    ...
                    <ul class="attr-list">
                      <li
                        v-for="(innerAttr,innerIndex) in predictAttr.innerAttrs"
                        class="attr"
                        :key="innerIndex"
                      >
                        <el-input
                          ...
                          @blur="getchangeInput(innerAttr)"
                        ></el-input>
                        <i
                          v-if="innerAttr.oldvalue"
                          class="el-icon-remove icon-btn danger"
                          @click="removeArrItem(predictAttr.innerAttrs,innerIndex)"
                        ></i>
                      </li>
                      <i
                        class="el-icon-circle-plus icon-btn primary"
                        @click="addInnerAttr(predictAttr)"
                      ></i>
                    </ul>
                  </li>
                </ul>
                <el-button size="small" @click="generatePredictGoods">确认规格</el-button>
              </div>
            </div>
          </div>
        </div>
        <div class="item">
          ...
          <div class="item-body no-flex">
            ...
            >
              ...
              ></el-table-column>
              <el-table-column align="center" prop="sellSize" label="已售"></el-table-column>
              <el-table-column align="center" min-width="120" prop="canSellSize" label="可售库存">
              ...
              <el-table-column align="center" min-width="120" prop="size" label="库存">
                <template slot-scope="scope">
              ...
              <el-table-column align="center" min-width="120" prop="price" label="价格(元)">
                <template slot-scope="scope">
              ...
              <el-table-column align="center" min-width="100" prop="state" label="状态">
                <template slot-scope="scope">
           ...
           <div class="form-section">
              ...
              <div class="section-body">
                 <div class="item" >
                  ...
                  <div class="item-body" style="line-height: 41px;">
                     <el-radio :disabled="pageAction ==='info'" v-model="infoGood.logisticsType" label="1">物流配送</el-radio>
                     <el-radio :disabled="pageAction ==='info'" v-model="infoGood.logisticsType" label="2">支持自提</el-radio>
                     <el-radio :disabled="pageAction ==='info'" v-model="infoGood.logisticsType" label="3">物流+自提</el-radio>
...
export default {
  ...
  methods: {
    ...
    for (const key in this.infoGood) {
      if (
        this.infoGood.hasOwnProperty(key) &&
        key.startsWith("pic") && this.infoGood[key] &&
        this.infoGood[key].trim() !== ""
      ) {
            ...
            res.object.guiGe.forEach(el => {
              if (person.map(ele => ele.attrs_id).indexOf(el.attrs_id) === -1) {
                let obj = {
                  ...
                  name: el.name,
                  innerAttrs:  res.object.guiGe.filter(
                    item => item.attrs_id === el.attrs_id
                  )
                };
                person.push(obj);
              }
    ...
    updateGoods() {
      this.$refs["goodsCommonForm"].validate(valid => {
        if (
          this.$refs["imageUploader"].files.length == 0 && this.goodImgs.length == 0
        ) {
    ...
    getchangeInput(item) {
      ...
      if (item.value_id) {
        let index = this.changeInputArr
          .map(el => el.value_id)
          .indexOf(item.value_id);
        if (index !== -1) {
    ...
    generatePredictGoods() {
      ...
      arr.forEach(ele => {
        ...
        for (var key in ele) {
          if (key.indexOf("attr_") !== -1) {
            let attr = this.predictAttrs.filter(
              el => el.attrs_id == key.split("_")[1]
            );
      ...
      var addArr = arr.filter(item => {
        if (
          this.attrGoods.filter(el => el.valueIdsStr === item.valueIdsStr)
            .length === 0
        ) {
          return item;
        }
    ...
    createGoods(res) {
      for (let i = 0; i < res.length; i++) {
        ...
        if (imageRes.ret) {
          ...
          this.$message.error(
            "第" + i + "张图片上传失败：" + imageRes.error.message
          );
          return false;
        }
      }
